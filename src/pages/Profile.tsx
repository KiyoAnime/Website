...
const Profile: Component = () => {
    ...
    const [usern, setUsern] = createSignal<string|undefined>();
    const [user, setUser] = createSignal<string|undefined>();
    const getUser = async () => {

    }
    return (
        <PageBlock title={`Kiyo • ${username}`}>
            <div class={'flex justify-center mt-12 md:justify-start sm:mt-10 md:mt-10'}>
                <div class={'flex flex flex-col items-center md:items-start'}>
                    <div class={'relative'}>
                        ...
                        <h1 class={'text-4xl font-bold mt-4 ml-64'}>{username}</h1>
                    </div>
                </div>
            </div>
        </PageBlock>
