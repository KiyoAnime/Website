		...
		<button class={classNames('inline-flex items-center justify-center px-5 h-10 text-center whitespace-nowrap text-white no-underline rounded-md', props.className, props.loading && 'cursor-not-allowed')} disabled={props.loading} type={props.type}>
			<Switch>
				<Match keyed={false} when={props.loading}>
					<svg class={'animate-spin text-gray-700 fill-gray-200 h-4 w-4'} viewBox={'0 0 100 101'}>
...
const RedBtn: Component<ParentProps<Props>> = (props) => (
	<Btn {...props} className={'bg-red-600 hover:bg-red-500'}/>
);

const BlueBtn: Component<ParentProps<Props>> = (props) => (
	<Btn {...props} className={'bg-blue-600 hover:bg-blue-500'}/>
);

const GreyBtn: Component<ParentProps<Props>> = (props) => (
	<Btn {...props} className={'bg-slate-600 hover:bg-slate-500'}/>
);

const TextBtn: Component<ParentProps<Props>> = (props) => (
	<Btn {...props} className={'bg-transparent hover:bg-slate-700'}/>
);

const GreenBtn: Component<ParentProps<Props>> = (props) => (
	<Btn {...props} className={'bg-green-700 hover:bg-green-600'}/>
);
